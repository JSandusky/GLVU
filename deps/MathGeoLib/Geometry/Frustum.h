...
public:
    ...
    /// JSandusky: added for configuring the frustum without recalculating the matrices for every single call.
    void ConfigurePerspectiveLH_OGL(vec pos, vec up, vec fore, float fov, float near, float far);
    ...
    // JSandusky
    /// Returns the screen-space AABB, XY in -1,1 and Z in 0,1 ranges
    AABB Project(const AABB& box) const;
    bool IsInsideFast(const AABB& box) const;
    void UpdatePlanes(bool doDupe = true);
};
