...
// Pylon modifications
    /// Return the division of Y by X
    inline float YOverX() const { return y / x; }
    /// Return the division of X by Y
    inline float XOVerY() const { return x / y; }

    inline bool operator==(const float2& rhs) const { return Equals(rhs); }
    inline bool operator!=(const float2& rhs) const { return !Equals(rhs); }

#define CLOSE_ENOUGH(lhs, rhs) ((lhs) + FLT_EPSILON >= (rhs) && (lhs) - FLT_EPSILON <= (rhs))
...
#undef CLOSE_ENOUGH
    ...
    /// Get the perpendicular vector in clockwise order.
    inline float2 PerpendicularClockwise() const { return float2(-y, x); }

    /// Get the perpendicular vector in counter-clockwise order.
    inline float2 PerpendicularCounterClockwise() const { return float2(y, -x); }

    inline float Cross(const float2& rhs) const { return x * rhs.y - y * rhs.x; }

    inline float2 Rotate(float degrees) const {
        float cosVal = cosf(degrees * 0.0174533f);
        float sinVal = sinf(degrees * 0.0174533f);
        float newX = x * cosVal - y * sinVal;
        float newY = x * sinVal + y * cosVal;
        return float2(newX, newY);
    }

    static const float2 PositiveX;
    static const float2 PositiveY;
    static const float2 NegativeX;
    static const float2 NegativeY;
};
