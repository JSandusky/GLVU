...
public:
  ...
  double to_double(double defVal = 0.0) const;
  float to_float(float defVal = 0.0f) const { return (float)to_double(defVal); }
  std::int64_t to_int64(std::int64_t defVal = 0) const;
  std::string to_string(const std::string& defVal = std::string()) const;
  bool to_bool(bool defVal = false) const { return type() == Type::BOOL ? (bool)*this : defVal; }
};
