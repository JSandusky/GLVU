...
extern "C" {
    ...
    // JSandusky: this function grabs all of the characters between two tokens, such as { and }
    // it does this by tracking the `depth` so that contents which use the tokens will still be captured currectly
    //    Input:
    //    - storageMax: maximum amount of characters accepted for the storage
    //    - openToken: character code for the opening block token
    //    - closeToken: character code for the closing block token
    //    Output:
    //    - storage: contents will be copied into this location
